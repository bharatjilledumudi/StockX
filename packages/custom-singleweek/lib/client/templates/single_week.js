...
Template[getTemplate('singleWeek')].created = function () {
  ...
  instance.getTerms = function () {
    ...
    var currentDate = (!!instance.data && !!instance.data.end) ? instance.data.end: Session.get('currentWeek');
    return {
      view: 'singleweek',
      after: moment(currentDate).subtract(7, 'days').startOf('day').toDate(),
  ...
  instance.autorun(function () {
    ...
    var currentDate = Session.get('currentWeek');
...
Template[getTemplate('singleWeek')].helpers({
  showDateNav: function () {
    return (typeof this.showDateNav === 'undefined') ? true : this.showDateNav;
  },
  singleWeekNav: function () {
    return getTemplate('singleWeekNav');
  },
  posts_list: function () {
